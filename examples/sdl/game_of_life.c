        ...
        for (usize y = 0; y < CANVAS_HEIGHT; ++y) {
            ...
            if (bj_bitmap_get(presentation_fb, x, y) != BACK_COLOR) {
                if (alive_neigh > 3 || alive_neigh < 2) {
                    bj_bitmap_put(draw_fb, x, y, BACK_COLOR);
                } else {
                    bj_bitmap_put(draw_fb, x, y, DRAW_COLOR_1);
                }
            } else {
                if (alive_neigh == 3) {
                    bj_bitmap_put(draw_fb, x, y, DRAW_COLOR_2);
                }
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    while (!quit) {
        ...
        if(need_update || painting) {
            SDL_UpdateTexture(texture, 0, bj_bitmap_data(presentation_fb), CANVAS_WIDTH * sizeof (u32));
