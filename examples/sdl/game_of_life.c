...
void draw(usize step) {
    bj_framebuffer_clear(draw_fb, BACK_COLOR);
        ...
        while (SDL_PollEvent(&e) != 0) {
            ...
            switch(e.type) {
                ...
                case SDL_MOUSEMOTION:
                    if(painting) {
                        usize x = ((float)e.button.x / (float)SCREEN_WIDTH) * CANVAS_WIDTH;
                        usize y = ((float)e.button.y / (float)SCREEN_HEIGHT) * CANVAS_HEIGHT;
                        bj_framebuffer_put(presentation_fb, x, y, BJ_COLOR_GREEN);
                    }
                    break;
