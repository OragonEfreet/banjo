...
static const usize bytes_value = sizeof(value_t);
...
TEST_CASE(set_a_value_with_new_key_growth_len_by_one) {
    ...
    for(key_t count = 1 ; count < 9 ; ++count) {
        ...
        REQUIRE_EQ(bj_hash_table_len(&htable), count);
    }
    bj_hash_table_reset(&htable);
}
...
TEST_CASE(set_a_value_with_existing_key_does_not_change_len) {
    ...
    bj_hash_table_set(&htable, &key, &value);
    REQUIRE_EQ(bj_hash_table_len(&htable), 1);
    bj_hash_table_reset(&htable);
}
...
TEST_CASE(set_a_value_with_new_key_returns_new_address) {
    ...
    for(key_t count = 1 ; count < 9 ; ++count) {
        const void* new_data = bj_hash_table_set(&htable, &count, &value);
...
TEST_CASE(get_nonexisting_key_returns_0) {
    ...
    const void* got = bj_hash_table_get(&htable, &nonexisting_key, 0);
    REQUIRE_NULL(got);
