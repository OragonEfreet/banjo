...
TEST_CASE(init_with_0_width_makes_a_nil_object) {
    ...
    bj_del(framebuffer, fb);
}

TEST_CASE(init_with_0_height_makes_a_nil_object) {
    bj_framebuffer* fb = bj_new(framebuffer, default, 10, 0);
    REQUIRE_EMPTY(bj_framebuffer, fb);
    bj_del(framebuffer, fb);
}

TEST_CASE(init_with_non_null_size_creates_a_buffer) {
    bj_framebuffer* fb = bj_new(framebuffer, default, 10, 12);
    REQUIRE_VALUE(fb->buffer);
    REQUIRE_EQ(fb->width, 10);
    REQUIRE_EQ(fb->height, 12);
    bj_del(framebuffer, fb);
}

TEST_CASE(default_init_sets_the_clear_color_to_black) {
    bj_framebuffer* fb = bj_new(framebuffer, default, 10, 10);
    REQUIRE_EQ(fb->clear_color, BJ_COLOR_BLACK);
    bj_del(framebuffer, fb);
}

TEST_CASE(resetting_a_framebuffer_sets_it_to_nil) {
    bj_framebuffer* fb = bj_new(framebuffer, default, 10, 10);
    bj_framebuffer_reset(fb);
    REQUIRE_EMPTY(bj_framebuffer, fb);
    bj_del(framebuffer, fb);
}

TEST_CASE(all_pixels_of_a_cleared_framebuffer_have_the_clear_color) {
    bj_framebuffer* fb = bj_new(framebuffer, default, 4, 4);

    bj_framebuffer_set_clear_color(fb, BJ_COLOR_AQUAMARINE);
    bj_framebuffer_clear(fb);

    for(usize p = 0 ; p < 16 ; ++p) {
        REQUIRE_EQ(fb->buffer[p], BJ_COLOR_AQUAMARINE);
    }
...
TEST_CASE(changing_clear_color_updates_the_clear_color_field) {
    bj_framebuffer* fb = bj_new(framebuffer, default, 1, 1);
    bj_framebuffer_set_clear_color(fb, BJ_COLOR_AQUAMARINE);
    REQUIRE_EQ(fb->clear_color, BJ_COLOR_AQUAMARINE);
    bj_del(framebuffer, fb);
}

TEST_CASE(the_data_can_be_retrieved) {
    bj_framebuffer* fb = bj_new(framebuffer, default, 10, 10);
    bj_color* data = bj_framebuffer_data(fb);
    REQUIRE_EQ(data, fb->buffer);
    bj_del(framebuffer, fb);
}
...
TEST_CASE_ARGS(draw_lines, {bj_pixel a; bj_pixel b; bj_color buf[25];}) {
    bj_framebuffer* fb = bj_new(framebuffer, default, 5, 5);

    bj_framebuffer_clear(fb);
    bj_framebuffer_draw_line(fb, test_data->a, test_data->b, BJ_COLOR_WHITE);
    int differences = memcmp(test_data->buf, fb->buffer, sizeof(bj_color) * 25);
...
TEST_CASE_ARGS(draw_triangles, {bj_pixel a; bj_pixel b; bj_pixel c;bj_color buf[25];}) {
    ...
    bj_framebuffer_clear(fb);
    bj_framebuffer_draw_triangle(fb, test_data->a, test_data->b, test_data->c, BJ_COLOR_WHITE);
    int differences = memcmp(test_data->buf, fb->buffer, sizeof(bj_color) * 25);
    REQUIRE_EQ(differences, 0);
...
/* static void buffer_content(bj_framebuffer* fb) { */
/*     bj_color* c = bj_framebuffer_data(fb); */
/*     for(usize p = 0 ; p < fb->width * fb->height ; ++p) { */
/*         printf("%c,", *(c++) == fb->clear_color ? 'B' : 'F'); */
/*     } */
/* } */
...
/*     bj_framebuffer* fb = bj_new(framebuffer, default, 5, 5); */
/*     bj_framebuffer_clear(fb); */
/*     bj_framebuffer_draw_line(fb, (bj_pixel){x0, y0}, (bj_pixel){x1, y1}, BJ_COLOR_WHITE); */
...
/*     bj_framebuffer* fb = bj_new(framebuffer, default, 5, 5); */
/*     bj_framebuffer_clear(fb); */
/*     bj_framebuffer_draw_triangle(fb, (bj_pixel){x0, y0}, (bj_pixel){x1, y1}, (bj_pixel){x2, y2}, BJ_COLOR_WHITE); */
...
int main(int argc, char* argv[]) {
    ...
    RUN_TEST(resetting_a_framebuffer_sets_it_to_nil);
    RUN_TEST(all_pixels_of_a_cleared_framebuffer_have_the_clear_color);
