...
// Glyph spacing as a percentage of glyph width.
// Pre-computed at compile time: max(1, round(0.1 * CHAR_PIXEL_W))
// With CHAR_PIXEL_W=8: round(0.8) = 1, max(1,1) = 1
...
static const struct bj_bitmap* get_charset_mask(struct bj_bitmap* p_bitmap)
{
    ...
    // Direct byte writes - mask is 8bpp, we control it, no need for API overhead
    for (size_t idx = 0; idx < charset_char_len; ++idx) {
        const size_t base_x = (size_t)CHAR_PIXEL_X((int)idx);
        ...
        for (int row = 0; row < CHAR_PIXEL_H; ++row) {
            ...
            for (int col = 0; col < CHAR_PIXEL_W; ++col) {
                if ((rowbits >> col) & 0x01u) {
                    mask_row[base_x + (size_t)col] = 0xFFu;  // full coverage
                }
            }
        }
    }
...
// =========================
// Fast fill - delegates to optimized format-specific functions
// =========================
static void fast_fill_rect(struct bj_bitmap* dst, const struct bj_rect* r, uint32_t color_native)
{
    ...
    const int x0 = r->x;
    const int y0 = r->y;
    const int x1 = x0 + (int)r->w;
    const int y1 = y0 + (int)r->h;

    // Dispatch to format-specific optimized fill (they handle clipping)
...
static void render_text_masked(
    ...
    // Spacing between glyph boxes in pixels (pre-computed constant)
    const int spacing = GLYPH_SPACING;
