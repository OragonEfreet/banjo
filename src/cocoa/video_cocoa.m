...
static void cocoa_dispatch_event(struct cocoa_window *window, NSEvent *event) {
  ...
  case NSEventTypeKeyDown: {
    // Ignore key repeat events (like X11, Win32, Emscripten)
    if ([event isARepeat]) {
      break;
    }
    ...
    while ((event = [NSApp nextEventMatchingMask:NSEventMaskAny
      ...
      if ([event window]) {
        ...
        if ([view isKindOfClass:[BanjoView class]]) {
          window = (struct cocoa_window *)objc_getAssociatedObject(view,
                                                                   "bj_window");
        }
      }
      ...
      if (window != 0) {
        ...
        NSEventType type = [event type];
        // Mark keyboard and mouse events as handled to prevent system beep
        if (type == NSEventTypeKeyDown || type == NSEventTypeKeyUp ||
            type == NSEventTypeLeftMouseDown ||
            type == NSEventTypeLeftMouseUp ||
            type == NSEventTypeRightMouseDown ||
            type == NSEventTypeRightMouseUp || type == NSEventTypeMouseMoved ||
      ...
      // Only send unhandled events to the system
      if (!handled) {
...
static int cocoa_get_window_size(const struct bj_window *p_abstract_window,
...
static void cocoa_delete_window(struct bj_window *p_abstract_window) {
  bj_check(p_abstract_window);
  @autoreleasepool {
    struct cocoa_window *p_window = (struct cocoa_window *)p_abstract_window;
    objc_setAssociatedObject(p_window->view, "bj_window", nil,
...
static void cocoa_renderer_configure(struct bj_renderer *renderer,
  ...
  @autoreleasepool {
    ...
    if (data->configured_view) {
      objc_setAssociatedObject(data->configured_view, "bj_sw_render", nil,
                               OBJC_ASSOCIATION_ASSIGN);
    }
...
cocoa_renderer_get_framebuffer(struct bj_renderer *renderer) {
  bj_assert(renderer);
  bj_assert(renderer->data);
  return &renderer->data->framebuffer;
}

static void cocoa_renderer_present(struct bj_renderer *renderer,
...
static void cocoa_destroy_renderer(struct bj_renderer *renderer) {
  bj_check(renderer);

  // This part will later depend on the renderer type
  if (renderer->data) {
    if (renderer->data->configured_view) {
      objc_setAssociatedObject(renderer->data->configured_view, "bj_sw_render",
...
static bj_bool cocoa_init_video(struct bj_video_layer *layer,
