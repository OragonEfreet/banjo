...
void system_init_window(
    ...
    bj_info("Backend: %s", p_create_info->name);
...
bj_window* bj_window_new(
    ...
    return s_backend->create_window(s_backend->p_data, p_title, x, y, width, height);
}
...
void bj_window_del(
    ...
    s_backend->delete_window(s_backend->p_data, p_window);
}
