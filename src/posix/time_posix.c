#include "posix.h"

#include <banjo/time.h>

#ifdef BJ_OS_POSIX

#include <time.h>

extern uint64_t bj_time_frequency_value;
static clockid_t   s_clock;
static uint64_t    s_timer_base;

void bj_end_time(void) {
    // EMPTY
}

void bj_begin_time(
    void
) {
    s_clock = CLOCK_REALTIME;
    bj_time_frequency_value = 1000000000;

#if defined(_POSIX_MONOTONIC_CLOCK)
    struct timespec spec;
    if (clock_gettime(CLOCK_MONOTONIC, &spec) == 0) {
        s_clock = CLOCK_MONOTONIC;
    }
#endif

    s_timer_base = bj_time_counter();
}

uint64_t bj_get_time(
    void
) {
    struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts);
    return (uint64_t) ts.tv_sec;
}


uint64_t bj_time_counter(
    void
) {
    struct timespec spec;
    clock_gettime(s_clock, &spec);
    return (uint64_t) spec.tv_sec * bj_time_frequency_value + (uint64_t) spec.tv_nsec;
}

uint64_t bj_time_frequency(
    void
) {
    return bj_time_frequency_value;
}

void bj_sleep(
    int milliseconds
) {
    struct timespec ts;
    ts.tv_sec = milliseconds / 1000;
    ts.tv_nsec = (milliseconds % 1000) * 1000000;
    nanosleep(&ts, NULL);
}

double bj_run_time(
    void
) {
    return (double)(bj_time_counter() - s_timer_base) / (double)bj_time_frequency();
}

#endif
