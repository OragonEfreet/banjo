...
static void dib_read_old(dib* p_dib, const u8* buffer, usize buffer_size, usize offset_check, bj_error** p_error) {
...
void dib_read_oldbmp(bj_oldbmp* p_bmp, const u8* buffer, usize buffer_size, usize data_offset, bj_error** p_error) {
    ...
    dib_read_old(&dib_data, buffer, buffer_size, data_offset, &p_inner_error);
    if (p_inner_error) {
        bj_forward_error(p_inner_error, p_error);
        return;
    }
