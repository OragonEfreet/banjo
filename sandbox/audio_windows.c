...
double make_noise(double time) {
	static const Note melody[] = {
		{659.25, 0.25}, {494.00, 0.125}, {523.25, 0.125}, {587.33, 0.25},
		{523.25, 0.125}, {494.00, 0.125}, {440.00, 0.25}, {440.00, 0.125},
		{523.25, 0.125}, {659.25, 0.25}, {587.33, 0.125}, {523.25, 0.125},
		{494.00, 0.25}, {494.00, 0.125}, {523.25, 0.125}, {587.33, 0.25},
		{659.25, 0.25}, {523.25, 0.25}, {440.00, 0.25}, {440.00, 0.25},
	};
	static const int melody_length = sizeof(melody) / sizeof(Note);

	// Calculate which note to play based on time
	double total_time = 0.0;
	for (int i = 0; i < melody_length; ++i) {
		if (time < total_time + melody[i].duration) {
			double local_time = time - total_time;
			return 0.5 * sin(2.0 * M_PI * melody[i].frequency * local_time);
		}
		total_time += melody[i].duration;
	}

	return 0.0; // silence after the melody
}
